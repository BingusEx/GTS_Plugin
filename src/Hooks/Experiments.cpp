...
namespace {
	...
	void CameraTest() {
		...
		if (isInThirdPerson) {
			float GlobalX = Runtime::GetFloatOr("cameraAlternateX", 1.0f);
			float GlobalY = Runtime::GetFloatOr("cameraAlternateY", 1.0f);
	...
	void Hook_Experiments::Hook(Trampoline& trampoline) { // This hook is usually commented out inside hooks.cpp
		...
		/*static FunctionHook<void(TESObjectREFR* ref)>DisableHook(
			...
			[](TESObjectREFR* ref) {
				...
				if (ref) {
					...
					if (AsActor) {
						if (get_visual_scale(AsActor) >= 2.0f) {
							return;
						}
		...
		static FunctionHook<void(bhkCharacterController* param_1, hkVector4* param_2, uintptr_t param_3)>sub_140DC0C90 (        
			...
			[](auto* param_1, hkVector4* param_2, auto param_3) {
				...
				param_2->quad.m128_f32[0] *= 0.0f;
				param_2->quad.m128_f32[1] *= 0.0f;
				param_2->quad.m128_f32[2] *= 0.0f;
				param_2->quad.m128_f32[3] *= 0.0f;
		...
		static CallHook<float(TESObjectREFR* param_1)>FUN_14080d560(        // rarely called             
			...
			[](auto* param_1) {
				...
				float Global = Runtime::GetFloatOr("cameraAlternateX", 1.0f);
				if (Global == 0.0f) {
					result = Runtime::GetFloatOr("cameraAlternateY", 1.0f);
					log::info("Altering hook 0");
				}
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		/*static CallHook<float(TESObjectREFR* param_1)>GetEyeHeight_140601E40(  // Get Eye Height, rarely called
			...
			[](auto* param_1) {
				...
				float result = GetEyeHeight_140601E40(param_1) * 10.0f;
