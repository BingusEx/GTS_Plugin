...
//-----------------------------------------Default Trigger Threshold Values
    const float Action_Sandwich = 4.0f; // used for sandwich only
    const float Action_AI_ThighCrush = 4.0f; // Used for AI only
	const float Action_Crush = 4.0f;
	const float Action_ButtCrush = 1.5f; // for butt and cleavage crush
	const float Action_Vore = 4.0f;
	const float Action_Grab = 4.0f;
	const float Action_Hug = 0.72f; // for hug grab/drop threshold 
    const float Action_FingerGrind = 4.0f;
