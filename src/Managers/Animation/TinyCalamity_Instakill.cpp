...
namespace {
    ...
    void GTS_LC_Start(AnimationEventData& data) { 
		...
		for (auto tiny: tinies) {
			if (tiny) {
				Runtime::PlaySoundAtNode("TinyCalamity_SpawnRune", tiny, 1.0f, 1.0f, "NPC Root [Root]");
	...
	void GTS_LC_Liftup(AnimationEventData& data) {
		Runtime::PlaySoundAtNode("TinyCalamity_ReadyRune", &data.giant, 1.0f, 1.0f, "NPC R Hand [RHnd]");
	...
	void GTS_LC_FingerSnap(AnimationEventData& data) { 
		...
		Runtime::PlaySoundAtNode("TinyCalamity_FingerSnap", &data.giant, 1.0f, 1.0f, "NPC R Hand [RHnd]");
		auto tinies = Animation_TinyCalamity::GetShrinkActors(&data.giant);
